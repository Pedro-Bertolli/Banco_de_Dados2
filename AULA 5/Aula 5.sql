...
-- Exemplo 2
SELECT C.nome_cliente, P.prazo_entrega, V.nome_vendedor FROM cliente C
	INNER JOIN pedido P
	ON C.codigo_cliente = P.codigo_cliente
...
-- Exemplo 1
SELECT p.descricao, p.unidade, ip.quantidade 
	FROM produto p LEFT OUTER JOIN item_pedido ip
...
-- Exemplo 1
SELECT p.descricao, p.unidade, ip.quantidade 
	FROM item_pedido ip RIGHT OUTER JOIN produto p
	ON  p.codigo_produto = ip.codigo_produto;
...
SELECT V.nome_vendedor, P.prazo_entrega, C.nome_cliente FROM cliente C
	FULL OUTER JOIN pedido P
	ON C.codigo_cliente = P.codigo_cliente
	FULL OUTER JOIN vendedor V
	ON P.codigo_vendedor = V.codigo_vendedor;
-- Quando são usadas mais de duas tabelas, é usado o FULL OUTER JOIN
